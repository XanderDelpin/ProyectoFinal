...
bool ListaDeCadenas::operator==(const ListaDeCadenas& unaLista) const
{
    ...
    if (equal) {
        for (int i = 0; i < this->cantidad && equal; i++) {
            if ((*this)[i] != unaLista[i]) {
                equal = false;
            }
        }
    }
...
{
    bool inserted = false;
    if (((this->cantidad + unaLista.cantidad) <= MAX_CAD) && (indice >= 0 && indice <= this->cantidad)) {
        for (int i = 0; i < unaLista.cantidad; i++) {
...
{
    MyString temp;
    return this->remover(indice, temp);
}

bool ListaDeCadenas::removerElPrimero()
{
    MyString temp;
    return this->removerElPrimero(temp);
}

bool ListaDeCadenas::removerElUltimo()
{
    MyString temp;
    return this->removerElUltimo(temp);
}
...
void ListaDeCadenas::ordenAscendente()
{
    int n = this->cantidad;
    MyString temp;
    bool switched = true;
...
void ListaDeCadenas::ordenDescendente()
{
    int n = this->cantidad;
    MyString temp;
    bool switched = true;
